...
export const CartList: FC<Props> = ({ editable = false }) => {
  ...
  return (
    <>
      {
        cart.map(product => (
          <Grid container spacing={2} key={product.slug + '_' + product.size} sx={{ mb: 1 }}>
            <Grid item xs={3}>
            ...
            <Grid item xs={7}>
              <Box display='flex' flexDirection='column'>
                ...
                {
                  editable
                    ? <ItemCounter currentValue={product.quantity} maxValue={10}
                      updateQuantity={(value) => { onNewQuantityValue(product, value) }} />
                    : <Typography variant='h5'>{product.quantity} {product.quantity > 1 ? 'productctos' : 'producto'}</Typography>
                }
